...
public class AclServiceWrapperServiceImpl implements AclServiceWrapperService {
    ...
    @Override
    public void createFullPermission(Object object) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        final Sid owner = new PrincipalSid(authentication);
        final Sid authoritySid = new GrantedAuthoritySid("ROLE_USER");
        ObjectIdentity oid = new ObjectIdentityImpl(object);
        MutableAcl acl = mutableAclService.createAcl(oid);
        acl.insertAce(acl.getEntries().size(), BasePermission.READ, authoritySid, true);
